...
AlarmGroup (N)
  ├─ GroupSettings           … 音・バイブ・スヌーズ等の共通設定（継承元 / D-010）
  ├─ DateRule[]              … グループの除外日 / 予約日
  └─ Alarm (N)
        ├─ AlarmSettings?    … null なら「グループから継承」
        └─ DateRule[]        … アラーム個別の除外日 / 予約日

DateOverride[]               … 除外日と予約日の衝突をユーザーが手動解決した結果（D-015）
HolidayCustomization[]       … 内蔵祝日データに対するユーザーの追加 / 除去（D-014）
DailySkip[]                  … その日限りのスキップ（D-008 の発動結果など）
...
| holidayBehavior | Enum | NONE / SKIP_HOLIDAY（祝日 + ユーザー定義の休日を含む） |
...
### DateOverride（衝突の手動解決結果 / D-015）

除外日と予約日が同じ日に重なったとき、ユーザーが選んだ答えを保存する。
既存の DateRule を書き換えないため、「12/29〜1/3 帰省」のような期間ルールの意図が壊れない。

| フィールド | 型 | 説明 |
|---|---|---|
| id | Long | 主キー |
| scope | Enum | APP / GROUP / ALARM（衝突を解決した階層） |
| ownerId | Long? | scope に対応する ID |
| date | LocalDate | 対象日（単日のみ。期間は日ごとに展開して保存） |
| decision | Enum | **RING**（鳴らす）/ **SILENT**（鳴らさない） |
| decidedAt | Instant | 解決した日時（UI で「手動決定」を示すため） |

### HolidayCustomization（祝日データへのユーザー編集 / D-014）

内蔵の祝日テーブルは**リソースとして持ち、DB には入れない**（アプリ更新で差し替えるため）。
ユーザーの編集だけを差分として DB に保存する。

| フィールド | 型 | 説明 |
|---|---|---|
| date | LocalDate | 対象日 |
| kind | Enum | **ADD**（この日を休日にする）/ **REMOVE**（内蔵祝日を休日扱いしない） |
| name | String? | 「お盆」「創立記念日」などの表示名 |
...
fun willRing(alarm: Alarm, date: LocalDate): Boolean {
  ...
  // --- ステップ 1: スイッチ（何があっても最優先で止まる） ---
  if (!alarm.isEnabled) return false
  if (alarm.group != null && !alarm.group.isEnabled) return false      // D-011 (AND判定)
  ...
  // --- ステップ 3: その日限りのスキップ ---
  if (dailySkipExists(alarm, date)) return false                        // D-008 / 「今日はスキップ」

  // --- ステップ 4: 手動で解決済みの衝突があればそれに従う（D-015） ---
  //     ALARM → GROUP → APP の順に探し、最初に見つかったもので確定。
  //     見つかった時点でステップ 5〜7 はすべてスキップする。
  findDateOverride(alarm, date)?.let { return it.decision == RING }
...
### 衝突検出のタイミング（双方向 / FR-4.5.1）

| 操作 | 検出内容 |
|---|---|
| 予約日を追加・編集した | その日付範囲に、自分または上位階層の除外日が掛かっていないか |
| 除外日を追加・編集した | その日付範囲に、自分または下位階層の予約日が掛かっていないか |
| グループやアプリ全体の除外日を追加した | 配下のアラームに予約日が無いか（影響するアラーム数も併せて提示する） |
| インポート実行後 | 全期間を走査し、未解決の衝突を一覧で提示する |
...
内蔵の祝日テーブルはアプリのリソースとして持ち、ユーザーの編集だけを DB に保存する。

```
fun isHoliday(date: LocalDate): Boolean {
  val custom = holidayCustomizations[date]
  if (custom?.kind == REMOVE) return false     // 内蔵祝日だが休日扱いしない（FR-4.3.6）
  if (custom?.kind == ADD)    return true      // ユーザーが追加した休日（FR-4.3.5）
  return builtInHolidays.contains(date)        // リソースの内蔵データ
}
```

| 論点 | 方針 |
|---|---|
| 内蔵データの形式 | 日付と名称の一覧をリソースとして持つ。振替休日も展開済みの状態で収録する |
| DB に入れない理由 | アプリ更新でデータを差し替えるとき、DB マイグレーションが不要になる |
| 収録期間 | 設定画面に「〜西暦 N 年まで収録」と表示する（FR-4.3.7） |
| 期限切れ対策 | 残り 1 年を切ったらアプリ更新を促す（FR-4.3.8）。更新しなくても手動追加で凌げる |
| 法改正への追従 | アプリ更新で対応。それまではユーザーが手動で ADD / REMOVE できる |
| 年末年始などの連休 | 期間を指定して ADD をまとめて生成する UI を用意する |
